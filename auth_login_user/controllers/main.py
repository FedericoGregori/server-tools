        ...
        if request.httprequest.method == "POST":
            ...
            try:
                ...
                if e.args == odoo.exceptions.AccessDenied().args:
                    ...
                    values["error"] = (
                        "User already logged in. Log out from "
                        "other devices and try again."
                    )
                    values["logout_all"] = True
                    values["failed_uid"] = (
                        failed_uid if failed_uid != SUPERUSER_ID else False
                    )
                else:
            ...
            if "error" in request.params and request.params.get("error") == "access":
                values["error"] = _(
                    "Only employee can access this database. "
...
class RootExt(odoo.http.Root):
    def get_response(self, httprequest, result, explicit_session):
        ...
        if not explicit_session and hasattr(response, "set_cookie"):
            response.set_cookie(
                "session_id", httprequest.session.sid, max_age=60 * 60, httponly=True
            )
