            ...
            if request.session.uid:
                ...
                def _update_user(u_sid, u_now, u_exp_date, u_uid):
                    ...
                    if u_uid and u_exp_date and u_sid and u_now:
                        request.env["res.users"].with_user(SUPERUSER_ID).browse(
                            u_uid
                        ).write(
                            {
                                "sid": u_sid,
                        ...
                        request.env["res.users"].with_user(SUPERUSER_ID).flush(
                            ["sid", "last_update", "exp_date", "logged_in"], user_pool
                        )
                ...
                if (
                    "options" in request_params
                    and "bus_inactivity" in request_params["options"]
                ):
